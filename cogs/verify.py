...
class Verify(commands.Cog):
    ...
    async def verify(self, ctx, key):
        ...
        if str(self.key) == key:
            ...
            await ctx.author.send(f"<@{ctx.author.id}>", embed=discord.Embed(description="You have been verified!", color=discord.Color.green(), delete_after=60).set_footer(text=f"Requested by {ctx.author.name} | This message will automatically delete itself after 60 seconds.", icon_url=ctx.author.avatar_url).set_author(name="Verified", icon_url="https://raw.githubusercontent.com/CriticRay/anix-images/main/checkmark-green.png?token=GHSAT0AAAAAABV3GFY562BKGYAAD6XVUZCYYXSUEMA"))
        else:
