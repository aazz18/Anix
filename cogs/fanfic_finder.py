...
class Fanfic_Finder(commands.Cog):
    ...
    async def check_if_user_in_database(self, ctx):
        ...
        if not data:
            return False
        return True
    @commands.command(name='fanfic_finder_set', aliases=['fanfic_finder_set_name'], brief='Set your fanfic preference')
    async def fanfic_finder_set(self, ctx, *, fanfic_type):
        """>fanfic_finder_set [fanfic_type]"""
        await self.create_database(ctx)
        if await self.check_if_user_in_database(ctx):
            await bad_info_channel(ctx, "You already have a fanfic preference", "", "")
            return
        conn = sqlite3.connect('data.db')
        c = conn.cursor()
    ...
    @commands.command(name='fanfic_finder', aliases=['"ff", "fanfic", "fanfic_search", "fanfic_search_finder", "fanfic_search_finder_finder"'], brief='Generates a random fanfic based on your taste with a random fanfic from fanfiction.net', description='Generates a random fanfic based on your taste with a random fanfic from fanfiction.net')
    async def fanfic(self, ctx):
        ...
        # get the user's fanfic preference
        conn = sqlite3.connect('data.db')
        c = conn.cursor()
        c.execute('''SELECT fanfic_name FROM fanfic_finder WHERE discord = ?''', (ctx.author.id,))
        data = c.fetchone()
