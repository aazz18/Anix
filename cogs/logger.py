...
class Logger(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            embed=discord.Embed(title=f"{message.author.name}#{message.author.discriminator}", description=f"in {message.guild.name}", color=discord.Color.blue()).set_thumbnail(url=message.author.avatar_url)
